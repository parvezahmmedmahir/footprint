...
// NEW: Advanced rejection detection structure
#[derive(Clone, Debug)]
...
pub struct FootprintConfig {
    pub show_rejection_zones: bool,
    pub rejection_threshold: f32, // Minimum strength to show
    pub highlight_large_orders: bool,
    pub large_order_threshold: f32,
...
impl Default for FootprintConfig {
    fn default() -> Self {
        Self {
            show_rejection_zones: true,
            rejection_threshold: 0.7,
            highlight_large_orders: true,
            large_order_threshold: 1000.0, // Minimum volume to consider as large order
...
pub struct KlineChart {
    ...
    pub footprint_config: FootprintConfig,
    rejection_zones: Vec<RejectionZone>,
    large_orders: Vec<(Price, f32, bool)>, // (price, volume, is_buy)
}
...
impl KlineChart {
        ...
        match basis {
            Basis::Time(interval) => {
                ...
                KlineChart {
                    ...
                    footprint_config: FootprintConfig::default(),
    ...
    fn detect_rejection_zones(&mut self, visible_earliest: u64, visible_latest: u64) {
        ...
        self.rejection_zones = new_zones;
    }
    ...
    fn analyze_datapoint_for_rejection(&self, timestamp: u64, kline: &Kline, footprint: &KlineTrades, zones: &mut Vec<RejectionZone>) {
        ...
        for (price, group) in &footprint.trades {
            let volume_ratio = if group.sell_qty > 0.0 {
                group.buy_qty / group.sell_qty
            } else {
                group.buy_qty / 0.1 // Avoid division by zero
            };

            let strength = self.calculate_rejection_strength(price, kline, group, total_volume);

            if strength >= self.footprint_config.rejection_threshold {
                let zone_type = if volume_ratio > 1.5 {
                    RejectionType::BuyerRejection
                } else if volume_ratio < 0.67 {
                    RejectionType::SellerRejection
                } else {
                    RejectionType::KeyLevel
                };

                zones.push(RejectionZone {
                    price_level: *price,
                    strength,
                    volume_ratio,
                    timestamp,
        ...
        if self.footprint_config.highlight_large_orders {
            // This requires mutable access which we can't do in this immutable loop
            // We'll handle large orders in a separate pass or change architecture later
    ...
    fn calculate_rejection_strength(&self, price: &Price, kline: &Kline, group: &data::chart::kline::GroupedTrades, total_volume: f32) -> f32 {
        ...
        let volume_ratio = group.total_qty() / total_volume;
        strength += volume_ratio.min(1.0) * 0.2;

        // Buy/Sell imbalance factor (30% weight)
        let imbalance_ratio = (group.buy_qty - group.sell_qty).abs() / group.total_qty().max(0.1);
        strength += imbalance_ratio * 0.3;

        // Price position factor (30% weight)
        ...
        let delta_strength = (group.delta_qty().abs() / group.total_qty().max(0.1)).min(1.0) * 0.2;
        strength += delta_strength;

        strength.min(1.0)
    }

    // NEW: Update footprint configuration
    pub fn update_footprint_config(&mut self, config: FootprintConfig) {
    ...
    pub fn set_basis(&mut self, new_basis: Basis) -> Option<Action> {
        ...
        match new_basis {
            ...
            Basis::Volume(_) | Basis::Range(_) => todo!(),
        }
    ...
    pub fn invalidate(&mut self, now: Option<Instant>) -> Option<Action> {
        // NEW: Detect rejection zones before rendering
        if let Some((visible_earliest, visible_latest)) = self.visible_timerange() {
            self.detect_rejection_zones(visible_earliest, visible_latest);
        }
...
fn draw_footprint_kline(
    ...
    let px_width = 1.0 / scaling;
    let wick_width = (1.0_f32).max(px_width);
...
fn draw_candle_dp(
    ...
    let px_width = 1.0 / scaling;
    let wick_width = (candle_width / 6.0).max(px_width);

    let wick_stroke = Stroke::with_color(
        Stroke {
            width: wick_width,
            ..Default::default()
        },
...
// NEW: Enhanced crosshair tooltip with rejection information
fn draw_crosshair_tooltip(
    ...
    if let Some(kline) = kline_opt {
        ...
        let mut segments = vec![
            ("O".to_string(), base_color, false),
            (kline.open.to_string(precision), change_color, true),
            ("H".to_string(), base_color, false),
            (kline.high.to_string(precision), change_color, true),
            ("L".to_string(), base_color, false),
            (kline.low.to_string(precision), change_color, true),
            ("C".to_string(), base_color, false),
            (kline.close.to_string(precision), change_color, true),
            (format!("{change_pct:+.2}%"), change_color, true),
        ];

        // Add rejection info if available
        if let Some(rejection) = current_rejection {
            segments.push(("Rej".to_string(), base_color, false));
            let rej_strength = format!("{:.0}%", rejection.strength * 100.0);
            let rej_color = match rejection.zone_type {
                RejectionType::BuyerRejection => palette.success.base.color,
                RejectionType::SellerRejection => palette.danger.base.color,
                RejectionType::KeyLevel => palette.warning.base.color,
            };
            segments.push((rej_strength, rej_color, true));
        }
        ...
        for (i, (text, seg_color, is_value)) in segments.iter().enumerate() {
            frame.fill_text(canvas::Text {
                content: text.to_string(),
            ...
            // Move to second line for rejection info
            if i == 8 && current_rejection.is_some() {
                x = position.x;
                y += 16.0;
            }
        }
...
// NEW: Enhanced drawing function for rejection zones
fn draw_rejection_zones(
    frame: &mut canvas::Frame,
    price_to_y: impl Fn(Price) -> f32,
    rejection_zones: &[RejectionZone],
    palette: &Extended,
    ...
    for zone in rejection_zones {
        let y = price_to_y(zone.price_level);
        ...
        frame.fill_rectangle(
            ...
            zone_color.scale_alpha(alpha),
        );

        // Draw rejection zone border
        ...
        frame.stroke(
            ...
            Stroke::with_color(border_stroke, zone_color.scale_alpha(0.8)),
        );

        // Draw rejection strength indicator
        if zone.strength > 0.8 {
            let indicator_size = 6.0 + (zone.strength * 4.0);
            let indicator_shape = match zone.zone_type {
                RejectionType::BuyerRejection => {
                ...
                RejectionType::KeyLevel => {
                    // Diamond for key levels
                    Path::new(|builder| {
                        builder.move_to(Point::new(-cell_width * 8.0, y - indicator_size));
                        builder.line_to(Point::new(-cell_width * 8.0 + indicator_size, y));
                        builder.line_to(Point::new(-cell_width * 8.0, y + indicator_size));
                        builder.line_to(Point::new(-cell_width * 8.0 - indicator_size, y));
                        builder.close();
                    })
                }
            ...
            frame.fill(&indicator_shape, zone_color);
        }
    }
}

// NEW: Enhanced large orders visualization
fn draw_large_orders(
    frame: &mut canvas::Frame,
    price_to_y: impl Fn(Price) -> f32,
    large_orders: &[(Price, f32, bool)],
    palette: &Extended,
    cell_width: f32,
) {
    for (price, volume, is_buy) in large_orders {
        let y = price_to_y(*price);
        let color = if *is_buy {
            palette.success.base.color
        } else {
        ...
        // Draw volume text for very large orders
        if *volume > 5000.0 {
            draw_cluster_text(
                frame,
                &abbr_large_numbers(*volume),
                Point::new(cell_width * 12.0 + order_size + 2.0, y),
                10.0,
                color,
                Alignment::Start,
                Alignment::Center,
