...
use iced::{
    ...
    widget::{
        button, column, container, image, pane_grid, pick_list, row, rule, scrollable, text,
...
fn main() {
    ...
    let _ = iced::daemon(LuxChart::new, LuxChart::update, LuxChart::view)
        ...
        .title(LuxChart::title)
        .theme(LuxChart::theme)
        .scale_factor(LuxChart::scale_factor)
        .subscription(LuxChart::subscription)
        .run();
}
...
impl LuxChart {
    ...
    fn view(&self, id: window::Id) -> Element<'_, Message> {
        ...
        let content = if id == self.main_window.id {
            ...
            let header_title = {
                let logo = image(image::Handle::from_bytes(style::LOGO_BYTES))
                    .width(18)
                    .height(18);
    ...
    fn restart(&mut self) -> Task<Message> {
        ...
        let (new_state, init_task) = LuxChart::new();
        *self = new_state;
